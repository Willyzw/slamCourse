...
def get_corresponding_points_on_wall(points,
    ...
    for point in points:
        isMatched = False
        if arena_left - eps < point[0] < arena_left + eps:
            point_ref = (arena_left, point[1])
            isMatched = True
        if arena_right - eps < point[0] < arena_right + eps:
            point_ref = (arena_right, point[1])
            isMatched = True
        if arena_bottom - eps < point[1] < arena_bottom + eps:
            point_ref = (point[0], arena_bottom)
            isMatched = True            
        if arena_top - eps < point[1] < arena_top + eps:
            point_ref = (point[0], arena_top)
            isMatched = True
                        
        if isMatched:
            left_list.append(point)
            right_list.append(point_ref)
