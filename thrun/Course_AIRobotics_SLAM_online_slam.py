...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(N-1):
        ...
        index = [0,1] + range(4,dim+2)
        Omega = Omega.expand(dim+2,dim+2,index,index)
        Xi = Xi.expand(dim+2,1,index,[0])
        ...
        # online
        index = range(2,dim+2)
        Omega2 = Omega.take(index,index)
        Xi2 = Xi.take(index,[0])
        A = Omega.take([0,1],index)
        B = Omega.take([0,1],[0,1])
        C = Xi.take([0,1],[0])
        
        Omega = Omega2 - A.transpose()*B.inverse()*A
        Xi = Xi2 - A.transpose()*B.inverse()*C
